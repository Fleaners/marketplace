...
const CHECKLIST_ITEMS = [
  ...
  { id: 'gstin_entry', label: 'Verify Indian GSTIN Number', description: 'Enable trade buyer tax benefits and secure premium trust banners.' },
  ...
  return (
    ...
    >
      <div className="space-y-6">
        ...
        <section className="rounded-[32px] bg-gradient-to-r from-accent-500/10 via-slate-900 to-slate-950 border border-slate-800 p-6 sm:p-8 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="space-y-3 text-center md:text-left flex-1">
            <span className="rounded-full bg-accent-500/10 px-3.5 py-1 text-xs font-bold text-accent-400 border border-accent-500/15">
              🚀 Setup Checklist
            </span>
            ...
            <p className="text-sm text-slate-300 max-w-xl leading-relaxed">
              Verify your physical warehouse address, input valid tax GSTIN info, and activate Buyer Protection to unlock premium search visibility and trade features.
            </p>
          </div>

          {/* Responsive SVG Circular Progress Circle */}
          <div className="flex items-center gap-4 bg-slate-950/80 border border-slate-800 px-6 py-4 rounded-3xl backdrop-blur-md">
            ...
            <div>
              ...
              <p className="text-xs font-medium text-accent-400 mt-1">
        ...
        <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
          ...
          >
            <span className="text-2xl">{profile.gstVerified ? '🛡️' : '🔒'}</span>
            <div className="space-y-0.5">
              <p className="text-xs font-extrabold uppercase tracking-wider">GST Verified</p>
              <p className="text-[10px] text-slate-400 font-medium">B2B Tax Credit</p>
            </div>
          </div>
          ...
          >
            ...
            <div className="space-y-0.5">
              ...
              <p className="text-[10px] text-slate-400 font-medium">National Directory</p>
            </div>
          </div>
          ...
          >
            ...
            <div className="space-y-0.5">
              ...
              <p className="text-[10px] text-slate-400 font-medium">Top Rank Search</p>
            </div>
          </div>
          ...
          >
            ...
            <div className="space-y-0.5">
              ...
              <p className="text-[10px] text-slate-400 font-medium">Escrow Trust Shield</p>
            </div>
          </div>
          ...
          >
            ...
            <div className="space-y-0.5">
              ...
              <p className="text-[10px] text-slate-400 font-medium">Direct Click-to-Chat</p>
            </div>
          </div>
        </section>
        ...
        <section className="grid gap-6 lg:grid-cols-[1fr_1.2fr]">
          ...
          <Card className="rounded-3xl border border-slate-800 bg-slate-950 p-6 space-y-4">
            <div>
              ...
              <h3 className="mt-1 text-xl font-bold text-white">Interactive Onboarding</h3>
              <p className="text-xs text-slate-400 mt-1">
                Toggle completed steps below to instantly calculate your cockpit grade score and synchronize badges.
              </p>
            </div>

            <div className="divide-y divide-slate-900 overflow-y-auto max-h-[500px] scrollbar-thin scrollbar-thumb-slate-900 pr-1">
              {CHECKLIST_ITEMS.map((item) => {
                ...
                return (
                  <div
                    ...
                    className="flex items-start gap-3 py-3.5 cursor-pointer hover:bg-slate-900/10 px-2 rounded-2xl transition-colors group"
                  >
                    <div className="pt-0.5">
                      <div
                        className={`h-5 w-5 rounded-md border flex items-center justify-center transition-all ${
                          isChecked
                            ? 'bg-accent-500 border-accent-500 text-slate-950 scale-105'
                            : 'border-slate-800 bg-slate-900/40 group-hover:border-slate-700'
                        }`}
                      >
                        {isChecked && (
                    ...
                    <div className="space-y-1">
                      <h4
                        className={`text-xs font-bold leading-tight transition-colors ${
                          isChecked ? 'text-slate-400 line-through' : 'text-white group-hover:text-accent-400'
                        }`}
                      >
                        {item.label}
                      </h4>
          ...
          <Card className="rounded-3xl border border-slate-800 bg-slate-950 p-6 flex flex-col justify-between">
            <form onSubmit={handleSaveCTA} className="space-y-6">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">
                  Business Cockpit Setup
                </p>
                <h3 className="mt-1 text-xl font-bold text-white">Merchant Core Directory</h3>
                <p className="text-xs text-slate-400 mt-1">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1.5">
                    <label className="text-xs font-semibold text-slate-300">WhatsApp Inquiry Number</label>
                    <input
                      type="text"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <label className="text-xs font-semibold text-slate-300">Business E-mail Address</label>
                    <input
                      type="email"
                      value={profile.email}
                  ...
                  <div className="space-y-1.5">
                    <label className="text-xs font-semibold text-slate-300">GSTIN / VAT Tax Registration Code</label>
                    <input
                      type="text"
                      value={profile.gstin}
                      onChange={(e) => handleFieldChange('gstin', e.target.value)}
                      className="w-full rounded-2xl bg-slate-900 border border-slate-800 px-4 py-3 text-sm text-white focus:outline-none focus:border-accent-500 transition-colors uppercase font-mono"
                    />
                  </div>
                </div>
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-slate-300">Primary Physical Address / Warehouse Location</label>
                  <textarea
                    rows={3}
                    value={profile.address}
                    onChange={(e) => handleFieldChange('address', e.target.value)}
                    className="w-full rounded-2xl bg-slate-900 border border-slate-800 px-4 py-3 text-sm text-white focus:outline-none focus:border-accent-500 transition-colors leading-relaxed"
                  />
                </div>
              </div>
              ...
              <div className="pt-4 border-t border-slate-900/60">
                <Button
                  ...
                  variant="primary"
                  disabled={isSaving}
                  className="w-full justify-center rounded-2xl font-bold bg-accent-500 text-slate-950 shadow-[0_12px_30px_-6px_rgba(255,149,0,0.3)]"
                >
